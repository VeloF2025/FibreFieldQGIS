...
export default function AdminHomeDropReviewsPage() {
  ...
  const loadCaptures = async () => {
    ...
    try {
      ...
      log.error('Failed to load captures:', {}, "Page", error instanceof Error ? error : new Error(String(error)));
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredCaptures = captures.filter(capture => {
    ...
    if (filters.dateRange !== 'all') {
      const captureDate = capture.capturedAt ? new Date(capture.capturedAt) : new Date();
      const now = new Date();
  ...
  const handleApprove = async (captureId: string) => {
    try {
      await homeDropCaptureService.approveHomeDropCapture(captureId, 'System Admin');
      await loadCaptures();
      setSelectedCapture(null);
    } catch (error) {
      log.error('Failed to approve capture:', {}, "Page", error instanceof Error ? error : new Error(String(error)));
    }
  };

  const handleReject = async (captureId: string, reason: string) => {
    try {
      await homeDropCaptureService.rejectHomeDropCapture(captureId, 'System Admin', reason);
      await loadCaptures();
      setSelectedCapture(null);
    } catch (error) {
      log.error('Failed to reject capture:', {}, "Page", error instanceof Error ? error : new Error(String(error)));
    }
  };
  ...
  const exportData = () => {
    const csvContent = [
      ...
      ...filteredCaptures.map(c => [
        ...
        c.customerInfo?.name || '',
        c.capturedBy,
        c.capturedAt ? new Date(c.capturedAt).toLocaleString() : 'Not captured',
        c.approvalStatus,
        c.qualityScore || ''
      ])
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {paginatedCaptures.map((capture) => (
                      <tr key={capture.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 whitespace-nowrap">
                          <div className="flex items-center gap-1">
                            ...
                            <span className="text-sm">
                              {capture.capturedAt ? new Date(capture.capturedAt).toLocaleDateString() : 'Not captured'}
                            </span>
                          </div>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          <Badge className={getStatusColor(capture.approvalStatus || 'pending')}>
              ...
              <Tabs defaultValue="details" className="w-full">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-medium">
                        {selectedCapture.capturedAt ? new Date(selectedCapture.capturedAt).toLocaleString() : 'Not captured'}
                      </p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <Badge className={getStatusColor(selectedCapture.approvalStatus || 'pending')}>
                        {selectedCapture.approvalStatus}
                      </Badge>
                    </div>
                  </div>
                  
                  {selectedCapture.approval?.rejectionReason && (
                    <Alert>
                      <AlertCircle className="h-4 w-4" />
                      <AlertDescription>
                        <strong>Rejection Reason:</strong> {selectedCapture.approval?.rejectionReason}
                      </AlertDescription>
                    </Alert>
                  )}
                </TabsContent>
                ...
                <TabsContent value="history" className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2 text-sm">
                      <Clock className="h-4 w-4 text-gray-400" />
                      <span>Last Modified: {selectedCapture.updatedAt ? new Date(selectedCapture.updatedAt).toLocaleString() : 'Not available'}</span>
                    </div>
                    ...
                    {selectedCapture.approval?.approvedAt && (
                      <div className="flex items-center gap-2 text-sm">
                        <CheckCircle className="h-4 w-4 text-green-400" />
                        <span>Approved: {selectedCapture.approval?.approvedAt ? new Date(selectedCapture.approval.approvedAt).toLocaleString() : 'Not available'}</span>
                        <span>by {selectedCapture.approval?.approvedBy || 'Unknown'}</span>
                      </div>
                    )}
                  </div>
                </TabsContent>
